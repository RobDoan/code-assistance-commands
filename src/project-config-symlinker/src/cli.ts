...
import type { CLIOptions, ProcessingResult } from './types.js';
import ConfigLoader from './config-loader.js';
import PathValidator from './path-validator.js';
import SymbolicLinkManager from './symbolic-link-manager.js';
import UserPrompt from './user-prompt.js';
import { Logger, ErrorHandler, ConfigSymlinkerError, ErrorCode } from './error-handler.js';
...
async function setupCommand(targetProjectPath: string, options: CLIOptions): Promise<void> {
...
async function main(): Promise<void> {
  ...
  program
    ...
    .action(async (targetProjectPath: string, options: CLIOptions) => {
      try {
        await setupCommand(targetProjectPath, options);
